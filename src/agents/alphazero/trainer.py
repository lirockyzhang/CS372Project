...
Each iteration:
    ...
    3. Evaluate   — new network vs old network (gating), vs uniform random,
                    and vs pure MCTS at a fixed sim budget (anchored benchmark).
    4. Replace    — accept new network if gating win rate >= --win-threshold.

Every iteration emits a metrics dict to a ``utils.training_logger.TrainingLogger``
that writes CSV + TensorBoard + console summaries in lockstep across all
trainers.

Checkpoints are saved after every iteration to a timestamped run directory.
"""
...
# ---------------------------------------------------------------------------
# Training step (kept module-level so it can be reused by load_mcts_dataset)
# ---------------------------------------------------------------------------
...
class AlphaZeroTrainer:
    """Runs the AlphaZero self-play / train / evaluate loop."""
    ...
    def __init__(
        ...
        # New evaluation knobs (the 4-metric strength panel)
        ...
        # Comparison-study knob: fire the strength panel (vs random + vs MCTS)
        # when cumulative NN forwards crosses each milestone instead of every
        # iteration. None = legacy "panel every iter" cadence.
        ...
        # Pre-load MCTS anchor dataset for in-memory random refresh.
        ...
        # Elo state — anchored against the previous accepted network, both
        # starting at INITIAL_ELO so the first gating delta is meaningful.
    ...
    # ------------------------------------------------------------------
    # Search-agent factories — let trainers swap PUCT for Gumbel cleanly
    # ------------------------------------------------------------------
    ...
    def run(self, start_iter: int = 0) -> None:
        ...
        print(f"\n{'='*65}")
        print(f"  AlphaZero Training  (search={self.search})")
        print(f"  device={self.device}  sims={self.sims}  "
        ...
        # Cumulatives for the comparison study. Reset each `run()` call --
    ...
    def _run_one_iteration(self, iteration: int) -> dict[str, Any]:
        ...
        # Update cumulative NN-forward and self-play-position counters.
        # AZ self-play does ~1 NN forward per MCTS sim (leaf eval), so
        # forwards_this_iter ≈ games × avg_game_length × sims_per_move.
        iter_forwards = int(
        ...
        # When eval_at_forwards is set, only run the strength panel
        # (vs random + vs MCTS@N) on iterations that cross a milestone --
        # the gating eval (vs previous) always runs because accept/reject
        # depends on it.
        run_panel = True
        ...
        # Elo update is always run on the gating result, even if rejected, so
        # the trend reflects whether the *attempted* network was stronger.
        ...
        if accepted:
            # Once we accept, the *new* network is the reference for the next
            # iteration's gating eval, so move the opponent rating forward.
    ...
    def _refresh_anchor_data(self) -> int:
        """Re-inject a random slice of the MCTS dataset into the buffer."""
    ...
    def _run_training(self) -> dict[str, Any]:
        # Cap to one epoch over the current buffer so noisy early policy
        # targets do not get re-fit dozens of times.
    ...
    def _run_evaluation(
        self,
        old_network: AlphaZeroNet,
        run_panel:   bool = True,
    ) -> dict[str, Any]:
        """Run gating eval (always) + strength panel (only when run_panel=True).

        ``run_panel=False`` skips the vs-random + vs-MCTS@N matches and emits
        ``None`` for those win-rate columns -- TrainingLogger writes them as
        empty CSV cells, which pandas reads as NaN. The cheap gating eval
        (vs previous accepted snapshot) always runs because the accept /
        reject decision depends on it.
        """
        self.network.eval()
    ...
    def load_checkpoint(self, path: str | Path) -> int:
        """Load a checkpoint. Optimizer / scheduler state are optional so
        lighter pretrain checkpoints (network weights only) work as a
        warm-start, not just full-training resumes."""
    ...
    def load_ppo_weights(self, path: str | Path) -> None:
        """Initialise the network from a PPO checkpoint (weights only)."""
